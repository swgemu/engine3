...
void RUDPProtocol::close() {
	...
	for (int i = 0; i < sendBuffer.size(); ++i) {
		RUDPPacket* pack = sendBuffer.get(i);
		delete pack;
	}
	...
	for (int i = 0; i < sequenceBuffer.size(); ++i) {
		RUDPPacket* pack = sequenceBuffer.get(i);
		delete pack;
	}
...
void RUDPProtocol::send(Packet* pack, bool doLock) {
	...
	try {
		if (isAvailable()) {
			...
			if (!DatagramConnector::send(pack))
...
void RUDPProtocol::run() {
	...
	try {
		if (isAvailable()) {
			RUDPPacket* pack = getNextSequencedPacket();
			if (pack == NULL) {
				unlock();
				return;
			}
...
bool RUDPProtocol::validatePacket(Packet* pack) {
	...
	if (seq < clientSequence) {
		...
		RUDPPacket* packet = new RUDPPacket(pack, seq);
		...
		RUDPPacket* oor = new OutOfOrderMessage(seq);
		sendPacket(oor);
...
void RUDPProtocol::resendPackets() {
	...
	for (int i = 0; i < MIN(sequenceBuffer.size(), 5); ++i) {
		RUDPPacket* packet = sequenceBuffer.get(i);
		...
		if (!DatagramConnector::send(packet)) {
			StringBuffer msg;
...
void RUDPProtocol::acknowledgeClientPackets(uint16 seq) {
	...
	try {
		...
		RUDPPacket* ack = new AcknowledgeMessage(seq);
...
void RUDPProtocol::requestNetStatus() {
	...
	try {
		...
		RUDPPacket* resp = new NetStatusRequestMessage(tick);
	...
	try {
		...
		if (!hasError) {
			...
			if (!clientDisconnected) {
				RUDPPacket* disc = new DisconnectMessage(this);
				prepareSend(disc);
				DatagramConnector::send(disc);
